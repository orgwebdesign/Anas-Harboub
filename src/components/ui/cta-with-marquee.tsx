...
function ScrambleButton({ onClick }: ScrambleButtonProps) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className="group px-8 py-3.5 bg-[#FF8A00] text-black font-extrabold rounded-full hover:bg-[#ffa026] hover:shadow-[0_0_30px_rgba(255,138,0,0.5)] transition-all cursor-pointer inline-flex items-center gap-3 text-sm sm:text-base"
    >
      <span>{displayText}</span>
      <div className="w-6 h-6 rounded-full bg-black text-[#FF8A00] flex items-center justify-center transition-transform group-hover:rotate-45">
...
export function HeroWithMarquee({ onOpenContact }: HeroWithMarqueeProps) {
  return (
    <div className="relative w-full rounded-[32px] bg-[#141519] border border-white/10 p-8 sm:p-12 overflow-hidden shadow-2xl my-12">
      ...
      <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-[#FF8A00]/10 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="container mx-auto relative z-10 text-center max-w-3xl space-y-6">
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold font-heading text-white leading-tight">
          Selected Web Design <span className="text-[#FF8A00]">Projects</span>
        </h2>
